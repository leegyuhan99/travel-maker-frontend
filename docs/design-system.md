...
| 상태 스타일 포함                 | 의미 있는 이름의 style `const`로 분리            |
| 긴 스타일 객체                   | 컴포넌트 상단 `const`로 분리                     |
| variant/size/tone 조합 존재      | `cva` 사용 검토                                  |
| 여러 컴포넌트에서 동일 패턴 반복 | 공통 컴포넌트, recipe, `cva` 검토                |
| 페이지/컨테이너급 긴 스타일      | 컴포넌트 분리 후 필요 시 `*.styles.ts` 분리 검토 |
| 디자인 값이 확정되지 않음        | 임의 token 추가 금지, TODO 작성                  |

Panda CSS 스타일이 길어진다는 이유만으로 바로 스타일 파일을 분리하지 않는다. 먼저 컴포넌트 책임을 나누고, 상태/비즈니스 로직을 분리한 뒤에도 스타일 정의가 과도하게 길다면 `*.styles.ts` 분리를 검토한다.

스타일 정리 우선순위:

1. UI 블록을 작은 컴포넌트로 분리한다.
2. 상태 관리, API 요청, 이벤트 핸들러 같은 비즈니스 로직을 hook 또는 feature-local 로직으로 분리한다.
3. 반복되는 스타일 패턴은 공통 컴포넌트, `cva`, recipe 사용을 검토한다.
4. 그래도 한 컴포넌트 안의 style const가 과도하게 많고 JSX 구조 파악이 어렵다면 `*.styles.ts` 분리를 검토한다.

`*.styles.ts` 분리를 검토하는 경우:

- 페이지/컨테이너 컴포넌트에서 style const가 많아 렌더링 구조를 파악하기 어려운 경우
- 하나의 파일에 상태 관리, API 요청, 이벤트 핸들러, 조건부 렌더링, 긴 스타일 객체가 모두 섞인 경우
- 동일한 스타일 패턴이 여러 컴포넌트에서 반복되는 경우
- 스타일 정의가 컴포넌트의 핵심 로직보다 더 큰 비중을 차지하는 경우
- 리뷰 시 JSX 구조 변경과 스타일 변경을 분리해서 보고 싶은 경우

작은 컴포넌트, 단순 카드, 태그, 버튼 수준의 스타일은 같은 파일에 유지해도 된다. `*.styles.ts` 분리는 파일을 나누는 것이 목적이 아니라 렌더링 구조와 스타일 책임을 더 읽기 쉽게 만드는 것이 목적이다.

스타일 정리 PR 기준:

- 기능 구현, API 연결, 스타일 대규모 정리는 가능하면 PR을 분리한다.
- 스타일 정리 중 UI 의도를 임의로 바꾸지 않는다.
- 스타일 파일 분리는 동작 변경 없이 진행한다.
- 스타일 분리와 컴포넌트 책임 분리가 함께 필요하면 먼저 작은 단위로 나누어 진행한다.
- 폴더/파일 위치 기준은 `docs/project-structure.md`를 따르고, 스타일 작성 기준은 이 문서를 따른다. 큰 컴포넌트를 분리할 때는 구조 기준과 스타일 기준을 함께 확인한다.
